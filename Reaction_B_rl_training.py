...
parser.add_argument('--core_smiles', type=str, default='OP1(Oc2c(c3c4ccccc4cc(*)c3O1)c5ccccc5cc2(*))=O')
...
seed_tl = 1234
batch_size = 128
reaction_dataset = pd.read_csv('./dataset/Reaction_b.csv')
augm = 100
drp_out = 0.0 
sigm_g = 0.3
...
othercomponents = 'COc1ccc(cc1)/C=N/C(=O)c1ccccc1.Sc1ccccc1'
...
real_comp = pd.read_csv('./dataset/Reaction_b_real.csv')
...
sc_path_ub = './result/' + 'Synthetic_score_Reaction_b_unbiased_' + str(args.agent_dataset) + '_' + str(args.trajectories_method)
output_path_ub = './result/' + 'Average_output_Reaction_b_unbiased_' + str(args.agent_dataset) + '_' + str(args.trajectories_method)

print("------------ Devoid of RL... Unbiased setting ---------")
smiles_unbiased, prediction_unbiased, generated_unbiased, molecule_list_unbiased, sc_unbiased = rl_adjustment(rl_agent, tl_Predictor_Reaction_b, args.core_smiles, n_to_generate, sc_path_ub, output_path_ub)


smile_pred_unbiased_df = dataframe_result(smiles_unbiased, prediction_unbiased, molecule_list_unbiased, sc_unbiased)
unbiased_result_path = './result/' + 'Reaction_b_unbiased_' + str(args.agent_dataset) + '_' + str(args.trajectories_method) + '.csv'
...
#Combinatorial_ub = combinatorial_evaluation_Reaction_b(molecule_list_unbiased, base_list, fluor_list, tl_Predictor_Reaction_b)


# RL training 

import reinforce_fragment_Reaction_b
from reinforce_fragment_Reaction_b import Reinforcement_random, Reinforcement_topp, Reinforcement_topk
...
def Reaction_b_get_reward(smiles, surrogate_regressor, invalid_reward=0.0):
...
if args.trajectories_method == 'random':
    RL_max = Reinforcement_random(rl_agent, tl_Predictor_Reaction_b, Reaction_b_get_reward, get_pred_val)
if args.trajectories_method =='topp':
    RL_max = Reinforcement_topp(rl_agent, tl_Predictor_Reaction_b, Reaction_b_get_reward, get_pred_val)
if args.trajectories_method =='topk':
    RL_max = Reinforcement_topk(rl_agent, tl_Predictor_Reaction_b, Reaction_b_get_reward, get_pred_val)
...
## If you want to see the results of combinatorial evaluation, run this section
#Combinatorial_bias = combinatorial_evaluation_Reaction_b(molecule_list_unbiased, base_list, fluor_list, tl_Predictor_Reaction_b)

print("------------ RL with sequential reward function ---------")

sc_path_bias = './result/' + 'Synthetic_score_Reaction_b_biased_' + str(args.agent_dataset) + '_' + str(args.trajectories_method)
output_path_bias = './result/' + 'Average_output_Reaction_b_biased_' + str(args.agent_dataset) + '_' + str(args.trajectories_method)


smiles_biased, prediction_biased, generated_biased, molecule_list_biased, sc_biased =rl_adjustment(RL_max.generator, tl_Predictor_Reaction_b,args.core_smiles, n_to_generate, sc_path_bias, output_path_bias)
smile_pred_biased_df = dataframe_result(smiles_biased, prediction_biased, molecule_list_biased, sc_biased)
biased_result_path = './result/' + 'Reaction_b_biased_' + str(args.agent_dataset) + '_' + str(args.trajectories_method) + '.csv'
smile_pred_biased_df.to_csv(biased_result_path, index =None)
plot_out(pred_Reinforce_max_plot, './result/' + 'Episodic_performance_Reaction_b_biased_' + str(args.agent_dataset) + '_' + str(args.trajectories_method))
