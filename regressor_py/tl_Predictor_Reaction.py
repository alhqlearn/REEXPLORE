...
def test_smiles_augmentation(df, N_rounds):
    ...
    for i in range(df.shape[0]):
        for j in range(N_rounds):
            ...
            dist_aug[output_col].append(df.iloc[i][output_col])
...
def fn_smiles_augmentation(df, N_rounds, noise):
    ...
    for i in range(df.shape[0]):
        for j in range(N_rounds):
            dist_aug['smiles'].append(randomize_smiles(df.iloc[i].smiles))
            dist_aug[output_col].append(df.iloc[i][output_col] + np.random.normal(0,noise))
    df_aug = pd.DataFrame.from_dict(dist_aug)
...
def pred_init(seed, batch_size, filename, current_path,augm, drp_out, sigm_g):
    ...
    data_clas = TextClasDataBunch.from_df(path, train_aug, valid, bs=bs, tokenizer=tok, 
                                              chunksize=50000, text_cols='smiles',label_cols=output_col, 
...
def test_performance(seed, batch_size, filename, train_aug, valid, current_path, drp_out, sigm_g, model_filename):
    ...
    for i in range(4):
        ...
        test_db = (TextList.from_df(df_aug, path, cols='smiles', processor=[tok_new, num_new]).split_from_df(col='valid').label_from_df(cols=output_col, label_cls=FloatList).databunch(bs=bs))
    ...
    test_db = (TextList.from_df(df, path, cols='smiles', processor=[tok_new, num_new]).split_from_df(col='valid').label_from_df(cols=output_col, label_cls=FloatList).databunch(bs=bs))
