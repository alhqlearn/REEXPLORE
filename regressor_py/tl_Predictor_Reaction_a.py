...
def test_performance(seed, batch_size, filename, train_aug, valid, current_path, drp_out, sigm_g): 
    ...
    for i in range(4):
        ...
        test_db = (TextList.from_df(df_aug, path, cols='smiles', processor=[tok_new, num_new]).split_from_df(col='valid').label_from_df(cols='yield', label_cls=FloatList).databunch(bs=bs))

        learner = text_classifier_learner(test_db, AWD_LSTM, config=None, pretrained=False, drop_mult=drp_out, metrics = [r2_score, rmse])

        learner.load(f'reg_Reaction_a'); 
    ...
    test_db = (TextList.from_df(df, path, cols='smiles', processor=[tok_new, num_new]).split_from_df(col='valid').label_from_df(cols='yield', label_cls=FloatList).databunch(bs=bs))
...
def predictor(smiles, seed, batch_size, filename, train_aug, valid, current_path, drp_out, sigm_g):
    ...
    test_db = (TextList.from_df(df_smile, path, cols='smiles', processor=[tok_new, num_new]).split_from_df(col='valid').label_from_df(cols='yield', label_cls=FloatList).databunch(bs=bs))
