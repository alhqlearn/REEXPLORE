...
from fastai_gen import *
from fastai_gen.text import *
from fastai_gen.vision import *
from fastai_gen.imports import *
...
def pred_init(seed, batch_size, filename, current_path):
    ...
    lm_learner = language_model_learner(lm_vocab, AWD_LSTM, config = dict(emb_sz=400, n_hid=1152, n_layers=3, pad_token=1, bidir=False, output_p=0.1,
                              hidden_p=0.15, input_p=0.25, embed_p=0.02, weight_p=0.2, tie_weights=True, out_bias=True), drop_mult=0.0, pretrained=False)
    ...
    reg_learner = text_classifier_learner(data_clas, AWD_LSTM,  config = dict(emb_sz=400, n_hid=1152, n_layers=3, pad_token=1, bidir=False, output_p=0.4,
                                hidden_p=0.3, input_p=0.4, embed_p=0.05, weight_p=0.5), pretrained=False, drop_mult=0.0, metrics = [r2_score, rmse])
    reg_learner.load_encoder(f'lm_encoder')
    reg_learner.freeze()
    
    return reg_learner, train_aug, valid

"""Training the regressor with gradual unfreezing """
...
def train_reg_full_unfreeze(unf_unf, lr_unf, reg_learner_tr):

    

    reg_learner_tr.unfreeze()
    reg_learner_tr.fit_one_cycle(unf_unf, lr_unf, moms=(0.8,0.7))
    
    return reg_learner_tr
    
    
    
def test_performance(seed, batch_size, filename, train_aug, valid, current_path):
    ...
    for i in range(4):
        ...
        test_db = (TextList.from_df(df_aug, path, cols='smiles', processor=[tok_new, num_new]).split_from_df(col='valid').label_from_df(cols='yield', label_cls=FloatList).databunch(bs=batch_size))

        learner = text_classifier_learner(test_db, AWD_LSTM, config=None, pretrained=False, drop_mult=0.0, metrics = [r2_score, rmse])

        #learner.load(f'reg_learner_flourination_yield'); 
        
        learner.load(f'/content/gdrive/My Drive/RELEASE/checkpoints/TL_Predictor/reg_fluorination_best'); 
        #get predictions
    ...
    test_db = (TextList.from_df(df, path, cols='smiles', processor=[tok_new, num_new]).split_from_df(col='valid').label_from_df(cols='yield', label_cls=FloatList).databunch(bs=batch_size))
...
def predictor(smiles, seed, batch_size, filename, train_aug, valid, current_path):
    ...
    df_gen_smile.insert(1, 'yield', '0')
    df_gen_smile.insert(2, 'valid', True)
   
    df_smile = pd.concat([df,df_gen_smile])
    
    test_db = (TextList.from_df(df_smile, path, cols='smiles', processor=[tok_new, num_new]).split_from_df(col='valid').label_from_df(cols='yield', label_cls=FloatList).databunch(bs=batch_size))
